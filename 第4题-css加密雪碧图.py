...
def btoa(string):
    ...
    return str(str_url).split("b'")[1].split("'")[0]
...
cookies = {
    'Hm_lvt_c99546cf032aaa5a679230de9a95c7db': '1625557016,1625564177,1626074704,1626770540',
    'qpfccr': 'true',
    'no-alert3': 'true',
    'sessionid': '95f6eqv9f800gtgw0akay9npwsjw88eu',
    'Hm_lvt_9bcbda9cbf86757998a2339a0437208e': '1625451171,1625557020,1626075050,1626770542',
    'Hm_lpvt_9bcbda9cbf86757998a2339a0437208e': '1626770567',
    'Hm_lpvt_c99546cf032aaa5a679230de9a95c7db': '1626770572',
}
...
for page in range(1,6):
    ...
    response = requests.get('http://match.yuanrenxue.com/api/match/4', headers=headers, params=params, cookies=cookies, verify=False)
    result = response.json()
    js_fun = execjs.compile(js_code)
    img_number = js_fun.call("hex_md5",btoa(result['key']+result['value']).replace('=',''))
    ...
    for td in soup.find_all('td'):
        ...
        for img in td.find_all('img'):
            if img.attrs['class'][1] != img_number: #满足此条件的图片显示，反之不显示
                imgs.append([num_dict[img.attrs['src']],float(img.attrs['style'].split('left:')[1].split('px')[0])])
        r += img_to_num(imgs)
